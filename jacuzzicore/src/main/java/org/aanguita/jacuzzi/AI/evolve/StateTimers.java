...
public class StateTimers<S> implements TimerAction {
    ...
    private final Map<Predicate<S>, TimerAction> registeredStateTimers;

    private final Timer timer;

    private Predicate<S> timerSource;
    ...
    public synchronized void setStateTimer(S state, long millis, Runnable runnable) {
        setStateTimer(new CompareStatePredicate<>(state), millis, runnable);
    }

    public synchronized void setStateTimer(Predicate<S> stateCondition, long millis, Runnable runnable) {
    ...
    public synchronized void removeStateTimer(S state) {
        removeStateTimer(new CompareStatePredicate<>(state));
    }

    public synchronized void removeStateTimer(Predicate<S> stateCondition) {
    ...
    private void checkStateTimers() {
        ...
        if (timerSource != null && timerSource.test(state)) {
        ...
        for (Map.Entry<Predicate<S>, TimerAction> timers : registeredStateTimers.entrySet()) {
            if (timers.getKey().test(state)) {
    ...
    public synchronized Long wakeUp(Timer timer) {
        if (alive.get() && timerSource != null) {
            ...
            // corresponds to a condition that no longer exists -> stop timer
            return 0L;
        }
    }
