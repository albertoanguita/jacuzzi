...
public class RangeList<T extends Number & Comparable<T>> implements Serializable, Iterable<Range<T>> {
        ...
        while (!finished && i < ranges.size()) {
            ...
            switch (range.compareTo(oneRange)) {
                ...
                case LEFT_OVERLAP:
                    ...
                    ranges.set(i, (Range<T>) range.buildInstance(range.getMin(), oneRange.getMax()));
                    finished = true;
                    break;
                case EQUALS:
                case INSIDE:
                ...
                case RIGHT_CONTACT:
                    // merge our range with this one
                    ranges.set(i, (Range<T>) range.buildInstance(oneRange.getMin(), range.getMax()));
                    finished = true;
                    checkRightOverlap = true;
                    break;
            ...
            if (range.getMax() == null) {
                ...
                while (!finished && i < ranges.size()) {
                    ...
                    switch (insertedRange.compareTo(oneRange)) {
                        ...
                        case LEFT_CONTACT:
                        case LEFT_OVERLAP:
                            ranges.set(i, (Range<T>) insertedRange.buildInstance(ranges.get(i - 1).getMin(), oneRange.getMax()));
                            ranges.remove(i - 1);
                            finished = true;
                            break;
                        case INSIDE:
    ...
    public void add(Range<T>... ranges) {
        for (Range<T> range : ranges) {
            add(range);
        }
    }
    ...
    public void remove(Range<T> range) {
        ...
        if (index >= 0) {
            ...
            if (secIndex < ranges.size() &&
                ...
                ranges.set(secIndex, (Range<T>) range.buildInstance(min, max));
            }
            ...
            switch (range.compareTo(ranges.get(index))) {
                ...
                case LEFT_OVERLAP:
                    ...
                    ranges.set(index, (Range<T>) range.buildInstance(min, max));
                    break;
                ...
                case INSIDE:
                    ...
                    if (min1.compareTo(max1) <= 0) {
                        ranges.add(index, (Range<T>) range.buildInstance(min1, max1));
                        index++;
                    }
                    if (min2.compareTo(max2) <= 0) {
                        ranges.add(index, (Range<T>) range.buildInstance(min2, max2));
                    }
                    break;
    ...
    @SuppressWarnings("unchecked")
    public void remove(Range<T>... ranges) {
